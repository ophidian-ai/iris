...
| Sitemap | Likely present | WordPress default (not independently verified) |
| Robots.txt | Likely present | WordPress default (not independently verified) |
| LocalBusiness Schema | **Missing** | No structured data with address, hours, geo coordinates |
...
| Issue | Impact |
|-------|--------|
| Avada theme | One of the heaviest WordPress themes available; loads large CSS/JS bundles on every page regardless of features used |
...
| Plugin | Slider Revolution 6.7.38 | Recent version -- acceptable |
...
| Dev Environment | Leaking | dev.totalfitnessofcolumbus.net URLs visible on production -- potential security concern if dev site is less secured |
...
| Nutrition Counseling | Thin | Generic meal plan advice with no counselor credentials; reads more like a blog post than a service page |
...
| Dev environment URLs | Links to dev.totalfitnessofcolumbus.net on live site | Unprofessional; potential security exposure; broken links if dev site is down |
